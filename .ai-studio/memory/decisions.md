...
## 2026-07-08 — Vida de Secuaz: mecánica mínima para HP propia (Director Creativo + Game Designer)

Resuelve el vacío de diseño detectado por Architect al especificar H1.16 (nuevo comportamiento de Secuaces por Dramaturgia, ver más arriba "Secuaces del Enemigo: comportamiento en Dramaturgia"): para que la Dramaturgia pueda ejecutar criterios como "ataca el secuaz con más vida", los Secuaces necesitan HP propio, que hoy el motor no modela. El Director Creativo ya confirmó por AskUserQuestion que sí deben tener vida propia; esta entrada cierra el resto del diseño en paralelo a H1.15 (Aliados).

### 1. Targeting: el jugador SÍ puede dirigir daño explícitamente a un Secuaz
- **Qué:** cuando el jugador juega una carta de Ataque o activa una habilidad de daño con un único objetivo, puede elegir explícitamente entre el Enemigo o cualquier Secuaz válido en mesa como destino — no solo mediante efectos de carta que digan literalmente "daña a un Secuaz". Esto es simétrico a la redirección de daño hacia un Aliado propio (H1.15), pero en dirección contraria (jugador → Secuaz enemigo).
- **Diferencia deliberada con Aliados:** los Secuaces NO bloquean daño automáticamente como pasivo de defensa (a diferencia de los Aliados, que absorben sin calentamiento). Atacar al Enemigo directamente sigue siendo posible aunque haya Secuaces vivos en mesa — elegir atacar a un Secuaz es una decisión táctica del jugador, no una obligación impuesta por su presencia. Excepción: la keyword Defensor (ya prevista en H1.16 original) sigue forzando que ese Secuaz concreto deba ser atacado primero cuando está en mesa.
- **Por qué:** es la referencia explícita de Marvel Champions citada por el Director Creativo — el jugador elige atacar al villano o a un minion según convenga a su plan (limpiar amenazas vs. avanzar la carrera de daño principal). Que los Secuaces no bloqueen automáticamente evita convertirlos en un muro obligatorio que ralentice cada combate; su coste de oportunidad para el jugador es "cada turno atacando a un Secuaz es un turno que no golpea al Enemigo", que ya es suficiente tensión sin añadir bloqueo forzado.

### 2. Origen del valor de HP: campo fijo en la definición del Secuaz en catálogo
- **Qué:** cada Secuaz tiene su propio campo de vida máxima en su `CardDefinition` (o definición equivalente), igual que un Aliado ya define su propia vida hoy. No se calcula a partir de la vida del Enemigo, ni es un valor global compartido entre todos los Secuaces del juego.
- **Por qué:** coherencia directa con el patrón ya cerrado de Aliados (H1.15); permite que Secuaces distintos tengan personalidad de balance distinta (un Secuaz "chatarra" de 1 HP que muere de un golpe vs. un Secuaz "élite" de 8 HP) sin inventar un sistema nuevo.

### 3. Qué pasa cuando un Secuaz muere: sale de mesa inmediatamente, sin trigger por defecto
- **Qué:** un Secuaz cuya vida llega a 0 sale de mesa de inmediato. Deja de poder ser seleccionado por cualquier criterio de Dramaturgia (`ALL`, `RANDOM_ONE`, `SPECIFIC_DEFINITION`, `HIGHEST_PLANO_ATTACK`, `HIGHEST_LIFE`, `LOWEST_LIFE` — ver punto 4) y deja de aportar su efecto pasivo mientras estaba en mesa. Por defecto, morir NO dispara ningún efecto adicional — es tan simple como "sale de mesa y deja de existir para el motor".
- **Exceso de daño (Arrollar):** se reutiliza la misma keyword Arrollar ya definida para Aliados (H1.15) en vez de inventar una nueva. Si un ataque con Arrollar mata a un Secuaz, el exceso de daño se aplica al Enemigo. Sin Arrollar, el exceso se pierde — exactamente el mismo patrón que Aliado/Líder, en dirección Secuaz/Enemigo.
- **Puerta abierta explícita:** el catálogo puede definir un Secuaz concreto con un efecto "al morir" como dato propio de esa definición (ej. un Secuaz que explota e inflige daño al Líder al caer), igual que Enemigo/Escenario pueden definir condiciones de victoria alternativas como datos propios (decisión ya cerrada arriba). Esto NO es un sistema genérico de "on-death triggers" para el MVP — es simplemente no cerrar la puerta a que contenido futuro lo use vía datos, sin rediseñar el modelo.
- **Por qué:** mantiene el modelo tan simple como sea posible para el MVP (coherente con "just for fun", alcance de contenido acotado) mientras reutiliza mecánica ya validada (Arrollar) en vez de duplicar reglas.

### 4. Vocabulario de `minionBehavior` en Dramaturgia: se confirma y se amplía
- **Qué:** se confirma el vocabulario cerrado que propuso Architect — `ALL`, `RANDOM_ONE`, `SPECIFIC_DEFINITION`, `HIGHEST_PLANO_ATTACK` — y se añaden dos valores nuevos ahora que hay HP disponible: **`HIGHEST_LIFE`** (el Secuaz con más vida actual) y **`LOWEST_LIFE`** (el Secuaz con menos vida actual, típicamente el más cerca de morir). Ambos operan sobre vida ACTUAL, no vida máxima — es lo que el ejemplo del Director Creativo ("ataca el secuaz con más vida") necesita para ser expresable.

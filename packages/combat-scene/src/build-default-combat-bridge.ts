...
/** NUEVO H2.8 (spec §2.4) — `bridge` + `boardContext` (contexto de catálogo resuelto una vez que
 *  `BoardView` necesita para pintar, ya que ni `CombatBridge` ni `CombatStateSnapshot` exponen catálogo). */
 ...
 * llama a `bridge.getSnapshot()`) contra el contenido real 2×2×2, junto al `BoardViewContext` (H2.8) que
 * `BoardView` necesita para pintar. Sustituye la responsabilidad de `build-hello-engine.ts` (H2.1, eliminado
 * en H2.6): migra su lógica de construcción de catálogo/engine, pero devuelve `DefaultCombatSetup` (H2.3
 * `CombatBridge` + H2.8 `BoardViewContext`) en vez de un `CombatEngine` crudo + snapshot. Único consumidor
 * hoy: `main.ts` (harness standalone de `combat-scene`) — H2.9 sustituirá esta función por la construcción
...
export async function buildDefaultCombatBridge(): Promise<DefaultCombatSetup> {
  ...
  const nameLookup = buildNameLookup({ leader, enemy, catalog }); // @collector/domain-catalog, H2.8 spec §2.3
